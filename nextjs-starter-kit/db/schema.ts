...
export const user = pgTable("user", {
  ...
  identifiedWorkflows: jsonb("identified_workflows"),
...
export const instance = pgTable("instance", {
  ...
  generationMethod: text("generation_method").default("manual"),
  category: text("category"),
  difficulty: text("difficulty"),
  anonymizationStatus: text("anonymization_status").default("pending"),
  createdAt: timestamp("createdAt").notNull().defaultNow(),
  updatedAt: timestamp("updatedAt").notNull().defaultNow(),
});
...
export const interviewSession = pgTable("interview_session", {
  ...
  thresholdScore: integer("threshold_score").default(0),
...
export const langGraphExecution = pgTable("langgraph_execution", {
  id: text("id").primaryKey(),
  userId: text("user_id").notNull().references(() => user.id, { onDelete: "cascade" }),
  sessionId: text("session_id").references(() => interviewSession.id, { onDelete: "cascade" }),
...
export const contextCompaction = pgTable("context_compaction", {
  id: text("id").primaryKey(),
  userId: text("user_id").notNull().references(() => user.id, { onDelete: "cascade" }),
  originalLength: integer("original_length").notNull(),
  compactedLength: integer("compacted_length").notNull(),
  compressionRatio: real("compression_ratio"),
  skillsPreserved: integer("skills_preserved").default(0),
  workflowsPreserved: integer("workflows_preserved").default(0),
  createdAt: timestamp("createdAt").notNull().defaultNow(),
});
